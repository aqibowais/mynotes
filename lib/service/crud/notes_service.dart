...
//creating user in the DB
  ...
  Future<int> deleteAllNotes() async {
    ...
    return await db.delete(
        noteTable); //it will delete all the rows in the table(delete all the notes)and return the no.of rows
  }
  ...
  Future<DatabaseNote> updateNote({
    ...
    await getNote(
        id: note
            .id); //it will just return the exception couldnotfindnote so we dont need its value to be saved
    ...
    if (updateCount == 0) {
      ...
      return await getNote(id: note.id); //we will return the note else
    }
  }
}
